## Subtitle Sync GUI 字幕对齐珍妮机 v1.0.0

### 项目简介
Subtitle Sync GUI 字幕对齐珍妮机是一款基于 Windows 的字幕时间轴自动校正工具，主要用于一键修正因视频帧率不同、片源版本不同、片头片尾长度变化、广告删减、音画延迟或其他原因造成的字幕偏轴、错位和逐渐漂移问题。

#### 当你的字幕和手头的片源无法对齐，且你不知道字幕和影片的帧率时，字幕对齐珍妮机是非常合适的校对工具。
